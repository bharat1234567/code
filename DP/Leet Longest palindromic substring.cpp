...
string ByBmakhija_longestPalindrome(string s) {
    ...
    if(len <= 1)
    return s;  // if string input is "a" then it should return a only.
    ...
    int start = 0;      
    int substrlen = 1;
    // if nothing matches ex: adcb then it should return 1st character.
    ...
    for(int i=0; i < len; i++)
    {
        ...
        for (int j = 0; j < i; j++)
        {
            if(s[i] == s[j] && (matrix[i-1][j+1] == true || i - j < 2)) // current i and j char matches and maatrix[i-1][j+1] == true means all middl ch
            // make a palindromic seq 
            {
                matrix[i][j] = true;
                if(substrlen < i-j + 1) //that is length of currently detected palindrome seq is greater then only consider it
                {
                 start = j;
                 substrlen = i - j + 1;
                }
    ...
    return ByBmakhija_longestPalindrome(s);
}
